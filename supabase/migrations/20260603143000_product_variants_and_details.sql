...
begin
  ...
  loop
    ...
    v_variant_id := nullif(trim(coalesce(v_item ->> 'variantId', '')), '');
    v_variant_label := nullif(trim(coalesce(v_item ->> 'variantLabel', '')), '');
    ...
    if v_variant_id is not null then
      ...
      if v_variant is null then
        raise exception 'Product option is unavailable';
      end if;

      v_unit_price := coalesce((v_variant ->> 'price')::numeric, v_product.price);
      v_stock := coalesce((v_variant ->> 'stock')::integer, v_product.stock);
    end if;
    ...
    insert into public.order_items (order_id, product_id, product_name, unit_price, quantity, variant_id, variant_label)
    values (v_order_id, v_product.id, v_product.name, v_unit_price, v_quantity, v_variant_id, v_variant_label);
